...
const covid19ImpactEstimator = (data) => {
  ...
  const impactHospitalBeds = Math.floor(
    (35 / 100) * totalBeds - impactSevereInfectionsTime
  );
  const severeHospitalBeds = Math.floor(
    (35 / 100) * totalBeds - severeSevereInfectionsTime
  );
  ...
  const avgDuration = avgUSD * period;
  const impactDollarsFlight = Math.ceil(impactInfectionsTime * avgPOP) * avgDuration;
  const severeDollarsFlight = Math.ceil(severeInfectionsTime * avgPOP) * avgDuration;
  const result = {
    data: 'data',
    impact: {
